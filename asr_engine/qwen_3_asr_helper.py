...
from openvino.properties import hint as ov_hint
...
def _openvino_compile_config_chain() -> Tuple[Tuple[str, Dict[str, Any]], ...]:
    """
    编译子模型时依次尝试的配置。默认 PERFORMANCE 路径在部分平台（尤其 Apple Silicon +
    OpenVINO 2026 CPU 插件）上会触发 OneDNN matmul primitive 创建失败，ACCURACY 模式可规避。
    """
    return (
        ("default", {}),
        ("ACCURACY execution mode", {ov_hint.execution_mode: ov_hint.ExecutionMode.ACCURACY}),
    )
...
class OVQwen3ASRPipeline:
    ...
    def __init__(self, model_dir: str, device: str = "CPU"):
        ...
        # NOTE: Some OpenVINO builds do not accept pathlib.Path here and may
        # fail with a generic "object is not initialized" error.
        req_device = str(device).upper()
        try:
            available_devices = [str(d).upper() for d in self.core.available_devices]
        except Exception:
            available_devices = []
        print(f"OpenVINO available devices: {available_devices}")

        # For strict device names (e.g. CPU/GPU/NPU), fallback to CPU when
        # requested device is unavailable in the current runtime.
        if req_device not in ("AUTO", "MULTI", "HETERO") and available_devices and req_device not in available_devices:
            print(f"⚠️ Requested device '{req_device}' is unavailable, fallback to CPU")
            req_device = "CPU"

        def _compile_with_fallback(model_path: Path, model_alias: str):
            attempts = []
            candidates = [req_device]
            if req_device != "CPU":
            ...
            last_err = None
            for dev_name in ordered_candidates:
                model = self.core.read_model(str(model_path))
                for cfg_name, cfg in _openvino_compile_config_chain():
                    try:
                        compiled = self.core.compile_model(model, dev_name, cfg)
                        if cfg_name != "default":
                            print(
                                f"ℹ️ {model_alias}: 使用「{cfg_name}」编译 "
                                f"(规避部分 CPU 上 OneDNN matmul 失败)"
                            )
                        print(f"✅ Compiled {model_alias} on {dev_name}")
                        return compiled, dev_name
                    except Exception as e:
                        attempts.append(
                            f"{dev_name}/{cfg_name}: {type(e).__name__}: {e}"
                        )
                        last_err = e

            msg = (
                f"Failed to load {model_alias} from '{model_path}'. "
                f"Attempted devices={ordered_candidates}. "
                f"Details: {' | '.join(attempts)}. "
                "若错误含 matmul primitive，多为 OpenVINO/OneDNN 与当前 CPU 的组合问题，"
                "可尝试升级/降级 openvino 或设置 ONEDNN_VERBOSE=all 查看详情。"
            )
            raise RuntimeError(msg) from last_err

        self.audio_conv, used_device = _compile_with_fallback(conv_path, "audio_conv")
        self.audio_encoder, _ = _compile_with_fallback(enc_path, "audio_encoder")
        self.text_emb, _ = _compile_with_fallback(emb_path, "text_embedding")

        lm = self.core.read_model(str(lm_path))
        ...
        compiled_lm = None
        lm_attempts: List[str] = []
        lm_last_err: Optional[BaseException] = None
        for cfg_name, cfg in _openvino_compile_config_chain():
            try:
                compiled_lm = self.core.compile_model(lm, used_device, cfg)
                if cfg_name != "default":
                    print(
                        f"ℹ️ language_model: 使用「{cfg_name}」编译 "
                        f"(规避部分 CPU 上 OneDNN matmul 失败)"
                    )
                break
            except Exception as e:
                lm_attempts.append(f"{cfg_name}: {type(e).__name__}: {e}")
                lm_last_err = e
        if compiled_lm is None:
            raise RuntimeError(
                "Failed to compile language_model. "
                + " | ".join(lm_attempts)
            ) from lm_last_err
